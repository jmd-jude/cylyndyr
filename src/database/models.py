from sqlalchemy import create_engine, Column, String, DateTime, ForeignKey, JSON, Table, MetaData
...
class Connection(Base):
    ...
    type = Column(String, nullable=False)  # e.g., 'snowflake'
    config = Column(JSON, nullable=False)  # Stores connection details
    created_at = Column(DateTime, default=datetime.utcnow)
...
class SchemaConfig(Base):
    ...
    user_id = Column(String, ForeignKey('users.id'), nullable=False)
    config = Column(JSON, nullable=False)  # Stores the entire schema configuration
...
def init_db(db_url='sqlite:///cylyndyr.db'):
